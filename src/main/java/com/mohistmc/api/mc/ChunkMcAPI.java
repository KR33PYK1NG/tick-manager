...
public abstract class ChunkMcAPI {
    ...
    private static Optional<Chunk> _getChunkNow(ChunkHolder holder, LocationType type) {
        if (holder != null) {
            ...
            switch (type) {
                case BORDER: future = holder.getBorderFuture(); break;
                case TICKING: future = holder.getTickingFuture(); break;
                case ENTITY_TICKING: future = holder.getEntityTickingFuture(); break;
                default: future = null; break;
            }
            if (future != null) {
                return future.getNow(ChunkHolder.UNLOADED_CHUNK).left();
            }
        }
    ...
    private static Optional<ChunkHolder> _getChunkHolder(ServerWorld world, long cPos) {
        ...
        return Optional.ofNullable(((ChunkManagerEx) world.getChunkProvider().chunkManager).rmc$getChunkHolder(cPos));
    }
